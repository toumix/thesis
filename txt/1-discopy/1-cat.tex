...
Given a signature $\Sigma$, we construct the free category with sums $F^+(\Sigma)$ as follows.
Take the free category $F(\Sigma)$, then take the free commutative monoid over each homset, i.e. arrows $f : x \to y$ in $F^+(\Sigma)$ are \emph{bags} (also called \emph{multisets}) of arrows $f_i : x \to y$ in $F(\Sigma)$.
...
The free category with bubbles $F^b(\Sigma)$ generated by a signature $\Sigma$ and a pair $b_\dom, b_\cod : C_0 \to C_0$ is given by the free category $F(\Sigma^b)$ generated by $(\Sigma^b)_0 = \Sigma_0$ and $(\Sigma^b)_1 = \bigcup_{n \in \N} b(\Sigma, n)$
for $b(\Sigma, 0) = \Sigma$ and $b(\Sigma, n + 1) = \Sigma \cup \{ b(f) \ \vert \ f \in b(\Sigma, n) \}$.
