...
{\normalfont Abstract classes for objects, arrows, categories, functors and transformations.}
...
Since Python cannot statically check that arrow composition is well-typed, the best we can do is raise an \py{AxiomError} at runtime.
\end{python}
...
class Function:
    ...
    def then(self, other: Function | type) -> Function:
        if not isinstance(other, Function): return self.then(self.id(other))
        inside = lambda *xs: other(*tuple(self(*xs)))
        return Function(inside, self.dom, other.cod)

    def __call__(self, *xs): return self.inside(*xs)

    __lshift__, __rshift__ = then, lambda self, other:\
        other.then(self) if isinstance(other, Function) else self.id(other).then(self)
\end{minted}
\end{python}
...
class Matrix:
    ...
    def __init__(self, inside: list[list[dtype]], dom: int, cod: int):
        self.dom, self.cod, self.inside = dom, cod, [
            [cast(self.dtype, val) for val in row] for row in inside]
    ...
    __repr__ = lambda self: "Matrix[{}]({}, {}, {})".format(
        self.dtype.__name__, self.inside, self.dom, self.cod)
    __getitem__ = lambda self, key: self.inside[key]
    __lshift__, __rshift__ = then, lambda self, other:\
        other.then(self) if isinstance(other, Matrix) else self.id(other).then(self)
\end{minted}

Subscriptable types such as \py{list[list[int]]} and the \py{__class_getitem__} method are a new feature of Python \py{>=3.10}.
...
class Arrow:
    ...
    def then(self, *others: Arrow) -> Arrow:
        others = [  # This allows nice syntactic sugar for the identity.
            other if isinstance(other, Arrow) else self.id(other) for other in others]
...
For the axiom \py{f >> id(y) == f == id(x) >> f} to hold, we need to make sure that \py{f == Arrow(x, y, [f])}, i.e. a box is equal to the arrow with just itself as boxes.
...
We also need to fix \py{Box.upgrade = Arrow.upgrade}: when we compose a box then an arrow, we want to return a new arrow object, not a box.
...
class Functor:
    ...
    def __init__(self, ob, ar, dom, cod):
        ob, ar = (d if hasattr(d, "__getitem__") else Dict(d) for d in (ob, ar))
    ...
    def __call__(self, other: Ob | Arrow) -> Ob | Arrow:
        ...
        if isinstance(other, Arrow):
            return self.cod.ar.id(self(other.dom)).then(
...
In DisCoPy, free categories with sums are implemented by \py{Sum}, a subclass of \py{Box} with an attribute \py{terms: list[Arrow]} as well as its own \py{upgrade} method, which turns an arrow into the sum of just itself.
It is attached to the arrow with \py{Arrow.sum = Sum}.
The composition of a sum as the sum of the compositions of its terms, we also override \py{Arrow.then} so that \py{f >> (g + h) == Sum.upgrade(f) >> (g + h)} for any arrow \py{f}.
We define equality so that \py{f == Sum.upgrade(f)}, equality of bags of terms is implemented as equality of lists sorted by an arbitrary ordering.
...
class Arrow(cat.Arrow):
    def __add__(self, other):
        terms = [self.sum.upgrade(f).terms for f in (self, other)]
    ...
    def then(self, other: Arrow) -> Arrow:
        return self.sum.upgrade(self).then(other)\
...
DisCoPy functors interpret bubbles with \py{name = "method"} as the application of \py{method} in the codomain category.
...
class Functor(cat.Functor):
    def __call__(self, other):
        if isinstance(other, Bubble):
            return getattr(self.cod.ar, other.name)(
...
\begin{minted}{python}
Matrix._not = lambda self, _, _: self.map(lambda x: not x)
...
def model(data: dict[Proposition, bool]):
    return Functor(ob={Ob('x'): 1},
                   ar={p: [[data[p]]] for p in data},
                   dom=Category(Ob, Formula),
                   cod=Category(int, Matrix[bool]))
