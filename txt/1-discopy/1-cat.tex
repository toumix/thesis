...
\py{B} is like the built-in \py{bool} but with addition and multiplication defined as disjunction and conjunction.
\end{python}
...
By default, we fix \py{Matrix = Matrix[B]}, we can get real or complex matrices with \py{Matrix[float]} or \py{Matrix[complex]}.
...
def model(data: dict[Proposition, bool]):
    return Functor(ob={Ob('1'): 1},
                   ar={p: [[data[p]]] for p in data},
                   dom=Category(Ob, Formula),
                   cod=Category(int, Matrix[B]))
