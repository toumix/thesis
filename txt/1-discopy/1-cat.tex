...
Thus, implementing a category in Python means nothing more than subclassing the abstract classes \py{Ob} and \py{Arrow} of listing~\ref{listing:abstract classes}, and then checking that the axioms hold via some (necessarily non-exhaustive) software tests.
...
\begin{minted}{python}
class Composable:
    """ Allows syntactic sugar self >> other and self << other. """
    def cast(self, other):
        return other if isinstance(other, Composable) else self.id(other)

    __rshift__ = __rlshift__ = lambda self, other: self.then(self.cast(other))
