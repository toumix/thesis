...
\inputpython{txt/1-discopy/abstract-classes.py}
\end{python}
...
\inputpython{txt/1-discopy/cat.py}
\end{python}
...
class Gate(Circuit, Box):
    def __init__(self, name: str, n_qubits: int):
        Box.__init__(self, name, Ob(n_qubits), Ob(n_qubits))
...
class Functor:
    ...
    def __call__(self, other):
        ...
        if isinstance(other, Arrow):
            return ar_factory.id(self(other.dom)).then(
...
In DisCoPy, free categories with sums are implemented by \py{Sum}, a subclass of \py{Box} initialised by an attribute \py{Sum.terms: list[Arrow]}.
