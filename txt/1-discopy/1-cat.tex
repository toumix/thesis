...
\begin{python}
{\normalfont Implementation of $\mathbf{Mat}_\S$ with \py{int} as objects and \py{Matrix} as arrows.}
...
We also need to fix \py{Box.upgrade = Arrow.upgrade}, otherwise we would not be able to compose a box with a diagram.
...
class Arrow(cat.Arrow):
    def dagger(self):
        return Arrow(self.cod, self.dom, [box.dagger() for box in self.boxes[::-1]])
...
\begin{minted}{python}
Matrix.dagger = lambda self: Matrix(dom=self.cod, cod=self.dom, inside=[
    [self[j][i].conjugate() for j in range(self.dom)] for i in range(self.cod)])

Y = Gate('Y', 1)
F = Functor(ob={Ob('1'): 2}, ar={Y: [[0, -1j], [1j, 0]]})
assert F(Y >> Y.dagger()) == F(Id) == F(Y.dagger() >> Y)
\end{minted}
\end{example}
...
\begin{minted}{python}
class Bubble(Box):
    def __init__(self, inside: Arrow, dom: Ob, cod: Ob):
        self.inside, name = inside, "Bubble({}, {}, {})".format(dom, cod, inside)
...
\begin{example}
We can implement the formulae of propositional logic with boxes as propositions, composition as conjunction, sum as disjunction and bubble as negation.
...
\begin{minted}{python}
Matrix.__add__ = lambda self, other: Matrix(dom=self.dom, cod=self.cod, inside=[
    [x + y for x, y in zip(u, v)] for u, v in zip(self.inside, other.inside)])
Matrix.zero = lambda dom, cod: Matrix(dom=dom, cod=cod, inside=[
    [0 for _ in range(cod)] for _ in range(dom)])
Matrix.bubble = lambda self: Matrix(
    self.dom, self.cod, [[not x for x in row] for row in self.inside])
...
for a in [0, 1]: for b in [0, 1]:
    ...
    ev = lambda f: F(f)[0][0]  # This takes the scalar out of a 1x1 matrix.
