...
$C(x, y) = \{f \in C_1 \vert \dom(f) = x \land \cod(f) = y \}$ is called a \emph{homset} and we write $f : x \to y$ whenever $f \in C(x, y)$.
...
Thus, implementing a category in Python means nothing more than subclassing the  abstract classes \py{Object} and \py{Arrow} of figure~\ref{fig:abstract classes}, and then checking that the axioms hold via some (necessarily non-exhaustive) software tests.
...
\end{itemize}
Thus, implementing a functor in Python amounts to subclassing the \py{Functor} class of figure~\ref{fig:abstract classes} (and then implementing software tests to check that the axioms hold).
...
The \py{Transformation} class is given in figure~\ref{fig:abstract classes}, checking that a transformation is natural cannot be done formally in Python.
...
\begin{figure}[h]
\caption{Abstract Python classes for categories, functors and transformations.}
\caption*{Note that annotations with dependent types are not supported by any Python implementation yet.
As there was no hope to get Python to check that compositions are well-typed, DisCoPy has no type hints and raises an \py{AxiomError} at runtime instead.}
\inputpython{txt/1-discopy/classes.py}{1}{999}
...
For example, take the following commutative diagram in $\mathbf{Pyth}$:
\[ \begin{tikzcd}
\py{int}
...
\end{tikzcd}
\]
It denotes a functor $3 \to \mathbf{Pyth}$ from the finite category $3$ with three objects $\{ 0, 1, 2 \}$ and three non-identity arrow $f : 0 \to 1, g : 1 \to 2$ and $h : 0 \to 2$, with the only non-trivial composition $f \fcmp g = h$.
Thus, this commutative diagram is nothing more than the equation \py{sum(range(n)) = n * (n - 1) // 2}.
...
\begin{figure}[h!]
\caption{Outline of the \py{discopy.cat} module.}
\inputpython{txt/1-discopy/cat.py}{1}{999}
\label{fig:cat.py}
\end{figure}
...
Free categories are implemented in the most basic module of DisCoPy, \py{discopy.cat}, which is sketched in figure~\ref{fig:cat.py}.
...
The \py{Functor} class has two mappings \py{ob} and \py{ar} as attributes, from objects to objects and from boxes to arrows respectively.
...
\begin{example}
A typical DisCoPy script starts by defining some objects and boxes:
{\normalfont \begin{python}
x, y, z = map(Ob, "xyz")
...
assert F(f >> g >> h)\
    ...
    == g >> h >> f
\end{python}}
We can interpret our arrows as Python functions using a \py{python.Functor}:
{\normalfont \begin{python}
G = python.Functor(
...
\end{python}}
We can interpret our arrows as matrices using a \py{tensor.Functor}:
{\normalfont \begin{python}
H = tensor.Functor(
    ob={x: 1, y: 2, z: 2},
    ar={f: [0, 1], g: [[0, 1], [1, 0]], h: [1, 0]})
assert H(f >> g) == H(h) == [1, 0]
\end{python}}
...
Suppose we're given a binary relation $R \sub M \times M$, then we can construct a quotient monoid $M / R$ with underlying set the equivalence classes of the smallest congruence $(\sim_R)$ generated by $R$, that is:
\begin{itemize}
...
The third says that $(\sim_R)$ is closed under products, it is equivalent to the substitution axiom: if $y \sim_R y'$ then $xyz \sim_R xy'z$ for all $x, z \in M$.
...
DisCoPy implements free $\dagger$-categories by adding an extra Boolean argument \py{_dagger} to boxes and a method \py{Arrow.dagger}, shortened to the postfix \py{[::-1]}, which reverses the order of boxes and flips that Boolean elementwise.
A $\dagger$-functor is a functor between $\dagger$-categories which commutes with the dagger, they are implemented by adding a case to the code for functor application so that \py{F(box) = F(box[::-1])[::-1]} whenever \py{box._dagger} is true.

A category $C$ has \emph{sums}, or equivalently $C$ is \emph{enriched in monoids}, when it comes equipped with a monoid $(+, 0)$ on each homset $C(x, y)$ such that $f \fcmp 0 = 0 = 0 \fcmp f$ and $(f + f') \fcmp (g + g') = f \fcmp g + f \fcmp g' + f' \fcmp g + f' \fcmp g'$ for all arrows $f, g, f', g'$.
In DisCoPy, free categories with sums are implemented by a class \py{Sum}, subclass of \py{Box} with a list of arrows \py{Sum.terms} as attribute.
\py{Sum.then} is straightforward: the composition of a sum is the sum of the compositions of its terms.
Defining equality requires some extra care however: we want an arrow to be equal to the sum of just itself.
DisCoPy functors automatically respect sums, i.e. we have that $F(0) = 0$ and $F(f + g) == F(f) + F(g)$.
Thus, a formal sum of arrows can be interpreted as a concrete sum, e.g. of matrices.
...
For example, we can define a negation bubble $\mathtt{not}$ on the category $\mathbf{Mat}_\B$ of Boolean matrices: it is the identity on objects and sends each matrix to its entrywise negation.
The resulting syntax with bubbles is strictly more expressive than that of free categories alone: negation cannot be expressed as a composition, there is no matrix $n$ in $\mathbf{Mat}_\B$ such that $f \fcmp n = \mathtt{not}(f)$ for all matrices $f$.
