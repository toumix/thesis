...
$$\spider_{\phi, a, c + n}(x) \otimes x^b
\ \fcmp \ x^c \otimes \spider_{\phi', n + c, d}(x)
\s = \s \spider_{\phi + \phi', a + b, c + d}(x)$$
We also require that our spiders satisfy the \emph{special} condition $\spider_{0, 1, 1}(x) = \id(x)$.
Spiders owe their name to their arachnomorphic drawing, for example $\spider_{\phi, 2, 6}$ is drawn as a node (the head) and its wires (the eight legs of the spider, two of them menacing us):
...
\begin{itemize}
\item $\merge(x) = \spider_{0, 2, 1}(x)$ and $\unit(x) = \spider_{0, 0, 1}(x)$ form a commutative monoid,
TODO DRAW % \ctikzfig{img/hypergraph/monoid}
\item $\ttsplit(x) = \spider_{0, 1, 2}(x)$ and $\counit(x) = \spider_{0, 1, 0}(x)$ form a cocommutative comonoid,
TODO DRAW % \ctikzfig{img/hypergraph/comonoid}
\item $\ttsplit(x) \fcmp \merge(x) = \id(x)$, called the \emph{special} condition,
TODO DRAW % \ctikzfig{img/hypergraph/special}
\item $\merge(x) \otimes x \fcmp x \otimes \ttsplit(x) \s = \s x \otimes \merge(x) \fcmp \ttsplit(x) \otimes x$, called the \emph{Frobenius law}.
TODO DRAW % \ctikzfig{img/hypergraph/frobenius}
\end{itemize}
...
Indeed, given a commutative monoid $\merge(x) : x \otimes x \to x, \s \unit(x) : 1 \to x$ and a cocommutative comonoid $\ttsplit(x) : x \to x \otimes x, \s \counit(x) : x \to 1$ subject to the Frobenius law, we can construct $\spider_{a, b}(x) : x^a \to x^b$ by induction on the number of legs.
...
TODO DRAW % \ctikzfig{img/hypergraph/induction-base}
\item $\spider_{a + 2, b}(x) \s = \s \merge(x) \otimes x^a \ \fcmp \ \spider_{a + 1, b}(x)$,
TODO DRAW % \ctikzfig{img/hypergraph/induction-step}
\end{itemize}
Finally we define spiders with one input leg by induction on the output legs $b \in \N$:
\begin{itemize}
\item $\spider_{1, 0}(x) \s = \s \counit(x)$,
TODO DRAW % \ctikzfig{img/hypergraph/induction-base-one-legged}
\item $\spider_{1, b + 2}(x) \s = \s \spider_{1, b + 1}(x) \ \fcmp \ \ttsplit(x) \otimes x^b$.
TODO DRAW % \ctikzfig{img/hypergraph/induction-step-one-legged}
\end{itemize}
One can show that this satisfies the spider fusion law, again by induction on the legs~\cite[Lemma 5.20]{HeunenVicary19a}.
In this way, we can construct an infinite family of spiders from just the four boxes $\merge(x), \unit(x), \ttsplit(x), \counit(x)$ and a finite set of equations.
A spider is nothing more than a big product followed by a big co-product.
...
TODO DRAW % \ctikzfig{img/hypergraph/phase-homomorphism}
\item $\shift_\phi(x) \otimes x \ \fcmp \ \merge(x)
\ = \ \merge(x) \fcmp \shift_\phi(x)
\ = \ x \otimes \shift_\phi(x) \ \fcmp \ \merge(x)$,
TODO DRAW % \ctikzfig{img/hypergraph/phase-commute-product}
\item $\ttsplit(x) \ \fcmp \ \shift_\phi(x) \otimes x
\ = \ \shift_\phi(x) \fcmp \ttsplit(x)
\ = \ x \otimes \ttsplit(x) \ \fcmp \ \shift_\phi(x)$.
TODO DRAW % \ctikzfig{img/hypergraph/phase-commute-coproduct}
\end{itemize}
...
\begin{remark}
Given any (non-special) Frobenius algebra on an object $x$, we can show that $x$ is its own left and right adjoint.
...
\begin{example}
In any pivotal category, there is a (non-special) Frobenius algebra for every object of the form $x^\star \otimes x$ given by:
\begin{itemize}
\item $\merge(x^\star \otimes x) = x^\star \otimes \ttcup(x^\star) \otimes x$ and $\unit(x) = \ttcup(x)$,
TODO DRAW
\item $\ttsplit(x^\star \otimes x) = x^\star \otimes \ttcap(x) \otimes x$ and $\counit(x) = \ttcap(x^\star)$.
TODO DRAW
\end{itemize}
Due to the drawing of its comonoid, it is also called the \emph{pair of pants} algebra.
...
If $C$ is braided, we can also give an inductive definition of spiders for tensors.
...
\end{itemize}
and a construct a monoid in a symmetric way, then show that they satisfy the spider fusion equations for $x \otimes y$.
...
\begin{example}
TODO FOL formula as a diagram model as a functor
\end{example}

The equality of hypergraph diagrams reduces to hypergraph isomorphism, it will be discussed in section~\ref{section:related and future}.
