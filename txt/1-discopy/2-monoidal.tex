...
Given two functions \py{f, g} we can define their tensor $\py{f + g} = \py{lambda b, x:} \py{(b, f(x) if b else g(x))}$.
\end{example}
...
product = lambda x: foldl(lambda a, b: a * b, x, 1)
...
The optional \py{down: bool} allows to interchange \py{i - 1} and \py{i} instead. Although for now it is redundant, we will override it in a meaningful way in section~\ref{subsection:symmetric}.
...
class Diagram(cat.Arrow):
    ...
    def interchange(self, i: int, left=False, down=True) -> Diagram: ...
...
Id, sqrt2 = Circuit.id(qubit), Gate("sqrt(2)", n_qubits=0)
...
class Functor(cat.Functor):
    ...
    def __call__(self, other):
        if isinstance(other, Ty):
            return self.cod.ob(sum([self(x) for x in other.objects], []))
        if isinstance(other, Ob):
            result = self.ob[other] if other in self.ob else self.ob[Ty(other)]
            if isinstance(result, self.cod.ob): return result
            return self.cod.ob([result])  # Allows some nice syntactic sugar.
        if isinstance(other, Layer):
            left, box, right = other
            return id_left @ self(box) @ id_right
...
class Eval(Functor):
    def __init__(self):
        ob = {Ob('1'): 2}
        ar = {H: [[x / sqrt(2) for x in row] for row in [[1, 1], [1, -1]]],
...
def encode(diagram: Diagram) -> Encoding:
    return Encoding(diagram.dom, [(box, len(left)) for left, box, _ in diagram.layers])
...
def decode(encoding: Encoding) -> Diagram:
    ...
    for box, offset in encoding.boxes_and_offsets:
        ...
        diagram >>= Diagram.id(left) @ box @ Diagram.id(right)
    return diagram
...
def model(size, data: dict[Predicate, list[bool]]):
    Functor(ob={Ty('x'): size},
            ...
            cod=Category(list[int], Tensor[B]))
