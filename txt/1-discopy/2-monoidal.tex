...
class Function(cat.Function):
    ...
    def tuple(self, other: Function | list[type]) -> Function:
        # This allows some syntactic sugar called _whiskering_.
        if not isinstance(other, Function): return self.tuple(self.id(other))
        def inside(*xs):
    ...
    __matmul__, __rmatmul__ = tuple, lambda self, other:\
        other @ self if isinstance(other, Function) else self.id(other) @ self
\end{minted}
\end{python}
...
class Tensor:
    ...
    def then(self, other: Tensor) -> Tensor:
        if not isinstance(other, Tensor): return self.then(self.id(other))
        inside = Matrix.then(*map(Tensor.downgrade, (self, other))).inside
        return type(self)(inside, self.dom, other.cod)

    def tensor(self, other: Tensor | list[int]) -> Tensor:
        if not isinstance(other, Tensor): return self.tensor(self.id(other))
    ...
    def __repr__(self): return super().__repr__().replace("Matrix", "Tensor")
    __matmul__, __rmatmul__ = tensor, lambda self, other:\
        other @ self if isinstance(other, Tensor) else self.id(other) @ self
\end{minted}
\end{python}
