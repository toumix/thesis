...
class Tensor(Matrix):
    ...
    __repr__ = lambda self: super().__repr__().replace("Matrix", "Tensor")
\end{minted}
\end{python}
...
circuit = Ket(0) @ Ket(0) >> X @ Id >> CX >> Id @ X
...
class Diagram:
    ...
    def tensor(self, other):
        return Sum.upgrade(self).tensor(other)\
...
In section~\ref{subsection:hypergraph} we will introduce the diagrammatic syntax for arbitrary formulae.
\end{example}
