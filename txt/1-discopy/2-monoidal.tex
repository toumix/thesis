...
The equality of diagrams is the smallest congruence generated by the \emph{interchanger relation}:
$$
...
There remains only to define the tensor operation.
First, we define the \emph{whiskering} $f \otimes \id(z)$ of a diagram $f$ by an object $z \in \Sigma_0^\star$ on the right: we tensor $z$ to the right-hand side of each layer of $f$, i.e. $f \otimes \id(z) = (x_1, f_1, y_1 z) \fcmp \dots \fcmp (x_n, f_n, y_n z)$.
And symmetrically for the whiskering $\id(z) \otimes f$ on the left.
Now we can define the tensor $f \otimes g$ of two diagrams $f : x \to y$ and $g : z \to w$ by applying the interchange law $f \otimes g = f \otimes \id(z) \fcmp \id(y) \otimes g$.

Thus, we have defined a monoidal category $F(\Sigma)$.
...
class Ty(Ob):
    ...
    def tensor(self, *others: Ty) -> Ty:
        if not all(isinstance(other, Ty) for other in others):
...
It has methods \py{__matmul__} for whiskering on the right, \py{__rmatmul__} for whiskering on the left and \py{upgrade} for turning boxes into layers with empty types on the left and right.
...
Again, we have the \py{upgrade} method which takes an old \py{cat.Arrow} and turns it into a new object of type \py{Diagram}.
This means we do not need to repeat the code for identity or composition which is already implemented by \py{cat.Arrow}.
In turn, when the user defines a subclass of \py{Diagram} they only need to use the decorator \py{@Diagram.subclass} without repeating the code for identity, composition or tensor.

\begin{python}
{\normalfont Outline of the class \py{monoidal.Diagram}.}
\inputpython{txt/1-discopy/diagram.py}
\end{python}
...
\inputpython{txt/1-discopy/box.py}
\end{python}

\begin{python}
{\normalfont Outline of the class \py{monoidal.Functor}.}
\inputpython{txt/1-discopy/functor.py}
\end{python}
