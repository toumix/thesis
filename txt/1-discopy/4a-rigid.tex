...
\begin{itemize}
\item $\ttcup(x \otimes y) \quad = \quad y^l \otimes \ttcup(x) \otimes y \s \fcmp \s \ttcup(y)$,
\ctikzfig{img/rigid/nesting-cups}
\item $\ttcap(x \otimes y) \quad = \quad \ttcap(x) \s \fcmp \s x \otimes \ttcap(y) \otimes x^l$,
\ctikzfig{img/rigid/nesting-caps}
\item and $\ttcup(1) \quad = \quad \ttcap(1) \quad = \quad \id(1)$.
\end{itemize}
The first two equations are drawn as diagrams in a non-foo monoidal category, i.e. with wire for composite types and explicit boxes for equality.
The last two equations would be drawn as the equality of three empty diagrams.
...
\end{center}
When these two functors are naturally isomorphic, $C$ is called \emph{pivotal}: we can rotate diagrams by 360 degrees.
...
def nesting(box_factory):
    @classmethod
    def method(cls, x: Ty, y: Ty) -> Diagram:
        if len(x) == 0: return cls.id(Ty())
        if len(x) == 1: return box_factory(x, y)
        head = box_factory(x[0], y[-1])
...
Given a pivotal signature $\Sigma$ with objects of the form $\Sigma_0 \times (\Z / 2 \Z)$, the free pivotal category is the quotient $F^p(\Sigma) = F^r(\Sigma) / R$ of the free rigid category by the relation $R$ equating the left and right transpose of the identity for each type.
While the diagrams of free rigid categories can have snakes, those of free pivotal categories can have circles: we can compose $\cap(x) : 1 \to x^l \otimes x$ then $\cup(x^l) : x^l \otimes (x^l)^l = x^l \otimes x$ to form a scalar diagram called the \emph{dimension} of the system $x$.
...
If the category also has a dagger, we get a four-fold symmetry: the box, its dagger, its transpose and its dagger-transpose (also called its conjugate).
