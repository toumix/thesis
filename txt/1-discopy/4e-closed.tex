...
class Function:
    ...
    def curry(self, n=1, left=True) -> Function:
        ...
        dom, cod = (self.dom[n:], exp(self.cod, self.dom[:n])) if left\
            else (self.dom[:len(self.dom) - n],
                  exp(self.cod, self.dom[len(diagram) - n:]))
        return Function(inside, dom, cod)

    @staticmethod
    def ev(base: tuple[type, ...], exponent: tuple[type, ...],
           left=True) -> Function:
        inside = (lambda *xs: xs[-1](*xs[:-1])) if left\
            else (lambda f, *xs: f(*xs))
...
class Ty(monoidal.Ty):
    ...
    def cast(cls, old: monoidal.Ty) -> Ty:
        if len(old) == 1 and isinstance(old[0], Exp):
            return old[0]
        return cls(old.inside)
...
Ty.__lshift__, Ty.__rshift__ = Over, lambda self, other: Under(other, self)
