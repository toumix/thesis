...
The method \py{cast} applies a \py{compact.Functor} so that we have \py{cast(f.downgrade())} \py{== f} on the nose for any \py{f: Diagram} and \py{cast(f).downgrade()} is equal to any \py{f:} \py{compact.Diagram} up to the special commutative Frobenius axioms.
