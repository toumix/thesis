...
\subsection{Premonoidal diagrams}
...
\subsection{Hypergraph diagrams}
...
We define $\mathtt{spiders}(f \fcmp g)$ as the size of the quotient set $(\mathtt{spiders}(f) + \mathtt{spiders}(g)) / R$ under the relation given by $\mathtt{wires}(f)(1, 0, i) = \mathtt{wires}(g)(0, 0, i)$ for all $i \leq \vert y \vert$.
Concretely, this can be computed as the connected components of an undirected graph with $\mathtt{spiders}(f) + \mathtt{spiders}(g)$ vertices.
...
When the category has biproducts, it is sufficient to define a \emph{repetition operator} $\mathtt{repeat}_x : C(x, x) \to C(x, x)$.
In the category of finite sets and relations $\mathbf{Mat}_\B$, this coincides with the usual notion of reflexive transitive closure.

\begin{python}
{\normalfont Implementation of the trace for compact-closed categories.}
...
class Function:
    ...
    def trace(self, n=-1):
        assert self.dom[n:] == self.cod[n:]
        if self.is_additive: ...  # Symmetric to the multiplicative case.
        fixed = (self >> Function.discard(self.cod[:n]) @ self.cod[n:]).fix()
        return Function.copy(self.dom[:n])\
            >> self.dom[:n] @ fixed >> self >> Function.discard(self.cod[n:])
\end{minted}
\end{python}
...
\subsection{Hypergraph versus premonoidal}
...
The \py{downgrade} method calls \py{make_progressive} to construct a \py{rigid.Diagram} with explicit boxes for swaps and spiders.
The \py{upgrade} applies a \py{rigid.Functor} so that \py{upgrade(f.downgrade()) == f} on the nose for any \py{f: Diagram} and \py{upgrade(f).downgrade()} is equal to any \py{f: rigid.Diagram} up to the special commutative Frobenius axioms.
...
\begin{minted}{python}
def pushout(
    left_spiders: int, right_spiders: int, left_wires: list[int], right_wires: list[int]
...
class Diagram(Composable, Tensorable):
    ...
    def downgrade(self) -> rigid.Diagram: ...

    upgrade = staticmethod(rigid.Functor(
        ob=lambda x: Ty(x[0]),
...
Indeed, the \emph{double push-out} (DPO) rewriting of Ehrig et al.~\cite{EhrigEtAl73} allows to pattern match the left-hand side of an axiom in a hypergraph diagram and then compute the substitution with the right-hand side~\cite{BonchiEtAl20}.
This can be extended to the case of symmetric diagrams where we do not have arrows for the spiders, by adding a convexity condition to pattern matching~\cite{BonchiEtAl16}.
...
In languages such as Dutch and Swiss German, \emph{cross-serial dependencies} are counter examples where grammatical wires are allowed to cross, albeit in a restricted way.
Yeung and Kartsaklis~\cite{YeungKartsaklis21} showed that if we reorder the words, the diagrams for these cross-serial dependencies can always be rewritten in a planar way using naturality.
