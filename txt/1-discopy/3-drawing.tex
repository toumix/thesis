...
\begin{python}
{\normalfont Implementation of the reading algorithm of listing~\ref{listing:lgpp2diagram}.}
...
We use the \py{array} data structure of \py{numpy}~\cite{VanDerWaltEtAl11} for bitmaps.
Connected components are computed with \py{scikit-image}~\cite{WaltEtAl14}, we make use of their default ordering by lexicographic order of their top-left pixel.
\end{python}

\begin{example}
Suppose we start from the following picture, where the red pixels are boxes and the black pixels are wires:
\begin{center}
\includegraphics[scale=.5]{img/bitmap2diagram/start.png}
\end{center}
We compute the connected components of red and black bitmaps and count $\{ 1, \dots, 6 \}$ boxes and 8 wires:
\begin{center}
\includegraphics[scale=.5]{img/bitmap2diagram/boxes.png}
\hspace{10pt}
\includegraphics[scale=.5]{img/bitmap2diagram/wires.png}
\end{center}
$$
0 \to 3, \s
0 \to 1, \s
...
\begin{center}
\includegraphics[scale=.5]{img/bitmap2diagram/finish.png}
\end{center}
\end{example}

\begin{example}
Suppose we start from the following picture.
