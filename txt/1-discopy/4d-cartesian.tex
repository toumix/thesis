...
drawing.equation(add >> Diagram.copy(x), Diagram.copy(x @ x) >> add @ add)
drawing.equation(zero >> Diagram.copy(x), zero @ zero)
drawing.equation(add >> Diagram.discard(x), Diagram.discard(x) @ Diagram.discard(x))
drawing.equation(zero >> Diagram.discard(x), Diagram.id(Ty()))
drawing.equation(  # TODO DRAW
    copy >> minus @ x >> add, Diagram.discard(x) >> zero, copy >> x @ minus >> add)
\end{minted}

A Hopf algebra which is also a Frobenius algebra is necessarily trivial, i.e. isomorphic to the unit.

\begin{minted}{python}
drawing.equation(  # TODO DRAW
    Diagram.id(x),
    x @ zero >> x @ copy >> add @ x >> Diagram.discard(x) @ x,
    x @ zero @ zero >> Diagram.discard(x @ x) @ x,
    Diagram.discard(x) >> zero)
\end{minted}
\end{example}
...
Higher-order functions are arrows in \emph{closed categories} which are the topic of the next section while recursive functions are arrows in \emph{traced categories}, they will be mentioned in section~\ref{section:related and future}.
